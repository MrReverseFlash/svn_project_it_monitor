...
public class FormatUtil {
    ...
    public static double formatDouble(double value,int num){
        ...
        return b.setScale(num,java.math.BigDecimal.ROUND_HALF_DOWN).doubleValue();
    }

    public static String getStartTime(Long timeMillSec){
        if (timeMillSec == 0L) {
            return "unknown";
        } else {
            return (new SimpleDateFormat(CommConstants.DEFAULT_TIME_FORMAT)).format(new Date(timeMillSec));
        }
    }
